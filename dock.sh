...
case "${1:-}" in
  ...
  vllm-noparser)  # fallback if logs show "unknown parser" — drop reasoning/tool parsers
    ...
    docker run -d --name "$NAME" --gpus all --shm-size=16gb --network host \
      ...
      nvcr.io/nvidia/vllm:26.01-py3 \
      vllm serve /model --served-model-name nemotron-super --quantization nvfp4 \
    ...
    docker ps --filter "name=$NAME" --format '{{.Names}}  {{.Status}}'
    ;;
  vllm30)  # L2 fallback: 30B FP8 (use if 120B OOMs / too slow). Flags per playbook §L2.
    docker rm -f "$NAME" 2>/dev/null
    docker run -d --name "$NAME" --gpus all --shm-size=16gb --network host \
      -v "$MODELS/nemotron-3-nano-30b-fp8:/model" \
      nvcr.io/nvidia/vllm:26.01-py3 \
      vllm serve /model --served-model-name nemotron-super \
        --tensor-parallel-size 1 --max-model-len 4096 --gpu-memory-utilization 0.85 --trust-remote-code
    docker ps --filter "name=$NAME" --format '{{.Names}}  {{.Status}}'
    ;;
