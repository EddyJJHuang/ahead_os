...
```bash
# 1. 拉最新 Jiawen 分支
git fetch origin Jiawen && git checkout Jiawen && git pull
git log -1 --format='%h %s'        # 期望:623db07(或更新) "frontend: wire 4 panels ..."
...
| 证据抽屉出现 VPN/PTO | 跑的是旧代码 | `git log -1` 确认是 `623db07`+ |
